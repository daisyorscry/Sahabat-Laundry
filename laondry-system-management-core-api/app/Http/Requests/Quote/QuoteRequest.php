...
class QuoteRequest extends FormRequest
{
    ...
    // Cek salah satu weight_kg|qty harus diisi, dan cocok dg pricing_model
    public function withValidator($validator)
    {
        $validator->after(function ($v) {
            ...
            // ambil service_model utk validasi model-based
            $ids = array_values(array_unique(array_map(fn($r) => $r['service_id'] ?? null, $items)));
            $services = Service::whereIn('id', array_filter($ids))->get(['id', 'code', 'pricing_model'])
                ->keyBy('id');
